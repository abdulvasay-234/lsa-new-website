...
export function CertificatesPage() {
  ...
  const exactCertificate = normalizedNameQuery ? courseRecords.find((record) => record.certificateId.toLowerCase() === normalizedNameQuery) : undefined
  const suggestions = !selectedStudent && normalizedNameQuery ? courseRecords.filter((record) => record.name.toLowerCase().includes(normalizedNameQuery) || record.certificateId.toLowerCase().includes(normalizedNameQuery)) : []
  ...
  useEffect(() => {
    if (!exactCertificate || selectedStudent?.certificateId === exactCertificate.certificateId) return
    const selectionTimer = window.setTimeout(() => setSelectedStudent(exactCertificate), 0)
    return () => window.clearTimeout(selectionTimer)
  }, [exactCertificate, selectedStudent])
  ...
  const generateCertificate = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!course) return setMessage('Select a course to continue.'); const record = selectedStudent || exactCertificate; if (!record) return setMessage('Select a matching student or Certificate ID.'); setSelectedStudent(record); setGenerated(record); setTemplateError(false); setMessage('') }
