...
export function AboutPage() {
  return (
    <>
      ...
      <Section className="about-page-approach"><Container><div className="about-page-heading"><p className="section-marker">03 — THE LSA APPROACH</p><h2>From knowledge<br />to capability.</h2></div><ol className="about-approach-list">{homepageContent.philosophy.stages.map((stage, index) => <li key={stage.name}><span>{`0${index + 1}`}</span><div><h3>{stage.name}</h3><p>{stage.description}</p></div></li>)}</ol></Container></Section>

      <Section className="about-page-institution"><Container><div className="about-page-heading"><p className="section-marker section-marker-yellow">04 — LIET × LSA</p><h2>Backed by an institutional<br />learning ecosystem.</h2></div><div className="about-institution-layout"><div className="about-institution-block"><strong>LIET</strong><span>Institutional foundation</span><p>{siteInfo.institution.name}</p></div><div className="about-institution-cross" aria-hidden="true">×</div><div className="about-institution-block"><strong>LSA</strong><span>Practical technology learning</span><p>Lords Skill Academy</p></div></div><p className="about-institution-note">{siteInfo.institution.statement}</p><div className="about-institution-logo"><img src={siteInfo.institution.logo.src} alt={siteInfo.institution.logo.alt} width={siteInfo.institution.logo.width} height={siteInfo.institution.logo.height} /></div></Container></Section>
